...
trait NoticeUtilsTrait {
	 ...
	 * @return void
	 */
	protected static function ajax_error_response( $message, $status_code = 400, $context = '' ) {
		wp_send_json_error(
			array(
				'status'  => 'error',
				'context' => $context,
	 ...
	 * @return void
	 */
	protected static function expired_response( $context = '' ) {
		wp_send_json_error(
			array(
				'status'  => 'error',
				'context' => $context,
	 ...
	 * @return void
	 */
	public static function not_allowed_message() {
		self::error_message( esc_html__( 'Sorry, you are not allowed to perform this action.' ), false, '', false );
	}
		...
		<div class="<?php echo esc_attr( static::$plugin_info['classes_general'] . '-error-notice' ); ?> <?php echo esc_attr( static::$plugin_info['classes_general'] . '-notice' ); ?> position-relative">
			...
			if ( ! empty( $buttons ) ) :
				...
				<div class="notice-buttons-wrapper">
					...
					<a href="<?php echo esc_url( $button['link'] ); ?>" <?php echo esc_attr( ! empty( $button['new_tab'] ) ? 'target="_blank"' : '' ); ?> role="button" class="notice-button"><?php echo esc_html( $button['title'] ); ?></a>
					<?php endforeach; ?>
				</div>
			<?php endif; ?>
		</div>
