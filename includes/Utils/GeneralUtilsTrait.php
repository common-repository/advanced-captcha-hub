...
trait GeneralUtilsTrait {
	...
	public static function loader_icon( $icon_size = 'small', $additional_classes = '', $additional_styles = '' ) {
		?>
		<img <?php echo ( ! empty( $additional_classes ) ? 'class="' . esc_attr( $additional_classes ) . '"' : '' ); ?> style="<?php echo esc_attr( $additional_styles ); ?>"  src="<?php echo esc_url( admin_url( 'images/spinner' . ( ( 'big' === $icon_size ) ? '-2x' : '' ) . '.gif' ) ); ?>"  />
		<?php
	}
		...
		foreach ( $assets as $asset_file ) {
			...
			if ( ! empty( $asset_file['conditional'] ) ) {
				foreach ( $asset_file['conditional'] as $key => $value ) {
					if ( empty( $_GET[ $key ] ) || sanitize_text_field( wp_unslash( $_GET[ $key ] ) ) !== $value ) {
						continue;
					}
				}
			}
