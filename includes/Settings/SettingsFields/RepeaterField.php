...
class RepeaterField extends FieldBase {
	...
	public function prepare_repeater_subitem_settings_field( $subitem_key, $repeater_row_index ) {
		...
		$subitem_settings_field['id']             = ( ! empty( $subitem_settings_field['id'] ) ? $subitem_settings_field['id'] : $this->field['key'] ) . '-' . $subitem_key . '-' . $repeater_row_index;
		...
		$subitem_settings_field['value']          = ! empty( $this->field['value'][ $repeater_row_index ][ $subitem_key ] ) ? $this->field['value'][ $repeater_row_index ][ $subitem_key ] : $this->field['default_subitem'][ $subitem_key ]['value'];
		...
		<div id="repeater-item-<?php echo esc_attr( $this->field['key'] . '-' . $index ); ?>" <?php $this->field_classes( 'repeater-item position-relative' ); ?> >
			...
			<div class="container-fluid">
				...
				foreach ( $this->field['default_subitem'] as $subitem_key => $subitem_field ) {
					$subitem_field['id']             = ( ! empty( $subitem_field['id'] ) ? $subitem_field['id'] : $this->field['key'] ) . '-' . $subitem_key . '-' . $index;
	 ...
	 * @return mixed
	 */
	public function sanitize_repeater_field( $value, $settings, $variation_id = null ) {
		$settings              = array();
		$value                 = (array) $value;
