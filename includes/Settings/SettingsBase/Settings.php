...
use GPLSCore\GPLS_PLUGIN_WADVCPA\Settings\SettingsBase\SettingsUtilsTrait;
use GPLSCore\GPLS_PLUGIN_WADVCPA\Settings\SettingsBase\SettingsFormTrait;
use GPLSCore\GPLS_PLUGIN_WADVCPA\Settings\SettingsBase\SettingsSubmitTrait;
...
abstract class Settings extends Base {
	...
	public function ajax_get_repeater_item() {
		if ( ! empty( $_POST['nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), $this->id . '-settings-nonce' ) ) {
					...
					foreach ( $section_settings['settings_list'] as $setting_name => &$setting_arr ) {
						...
						if ( 'repeater' === $setting_arr['type'] ) {
							foreach ( $setting_arr['default_subitem'] as $repeater_field_name => &$repeater_field_arr ) {
								...
								$repeater_field_arr['key']    = ! empty( $repeater_field_arr['key'] ) ? $repeater_field_arr['key'] : $repeater_field_name;
							}
						}
					}
				}
